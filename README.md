...
---
**最後更新**：2026/06/26（legacy fallback 拆分至 ITCSS partials）
**版本**：1.3.29
...
- `v1.3.29`（2026/06/26）：依 ITCSS 規則拆分並刪除 `css/components/content/_legacy-main-fallback.scss`；將仍被指定頁面使用的樣式移入 base、layouts、navigation、content、widgets 對應 partial，並以檔頭註解標明來源、用途與套用範圍；`css/layouts/_layouts.scss` 的 grid class 收斂為目前 HTML 實際使用欄位，不再產生未使用的 12 欄預擴充 class。已重新編譯 `css/main.css`，並以瀏覽器檢查 `home`、`products`、`product-detail`、`checkout`、`checkout-success`、`member-center`、`blog`、`blog-detail`、`branches`、`faq` 的 `main.css` 載入與核心區塊尺寸；另驗證 `product-detail` 的漢堡、搜尋、購物車 drawer、登入 modal 與 modal close 互動可用。
