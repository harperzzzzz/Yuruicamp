...
│   ├── partials/                 # 七個功能模組的 HTML 片段（由 core.js 動態載入）
...
|------|---------|---------|---------|------|
| HTML 頁面 | 11 個 | 2 個（login + dashboard）+ 7 個 partials | 6 個 | 26 個 |
| JavaScript 模組 | 17 個（6 元件 + 10 頁面 + 1 主程式）| 8 個（1 core + 7 功能）| 5 個 | 30 個 |
| CSS 檔案 | 1 個（main.css）| 1 個（admin.css）| 1 個（booking.css）| 3 個 |
| Mock 資料 JSON | 5 個（data/）| 7 個（admin/data/）| 2 個（booking/data/）| 14 個 |
...
**最後更新**：2026/06/15（移除登入及註冊保留純第三方登入）
**版本**：1.2.9
...
- `v1.2.9`（2026/06/15）：移除登入及免費註冊功能，保留純第三方登入。
