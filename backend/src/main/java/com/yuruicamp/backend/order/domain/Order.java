...
public class Order {
	...
	@Column(name = "checkout_request_hash", length = 64) private String checkoutRequestHash;
	...
	public String getId(){return id;} public String getCustomerId(){return customerId;} public String getCheckoutIdempotencyKey(){return checkoutIdempotencyKey;} public String getCheckoutRequestHash(){return checkoutRequestHash;} public List<OrderItem> getItems(){return items;}
	...
	// 建立待付款訂單，並保存冪等鍵與請求指紋。
	public void initialize(String id,String customerId,String checkoutIdempotencyKey,String checkoutRequestHash,String buyerName,String buyerEmail,String recipientName,String shippingAddress,String shippingPhone,PaymentMethod paymentMethod,Instant now,Instant expiresAt){this.id=id;this.customerId=customerId;this.checkoutIdempotencyKey=checkoutIdempotencyKey;this.checkoutRequestHash=checkoutRequestHash;this.buyerName=buyerName;this.buyerEmail=buyerEmail;this.recipientName=recipientName;this.shippingAddress=shippingAddress;this.shippingPhone=shippingPhone;this.paymentMethod=paymentMethod;this.paymentStatus=PaymentStatus.unpaid;this.refundStatus=RefundStatus.none;this.status=OrderStatus.unshipped;this.placedAt=now;this.checkoutExpiresAt=expiresAt;this.subtotal=BigDecimal.ZERO;this.shippingFee=BigDecimal.ZERO;this.discount=BigDecimal.ZERO;this.total=BigDecimal.ZERO;}
