...
規範提供 AI 程式設計工具一份簡化、可直接實作的 Yuruicamp主站和 Booking 網站視覺合約。
它會保留目前戶外電商與露營訂房的品牌識別，同時將設計語言整理為可重複使用的設計 token、版面規則、元件類別與無障礙檢查項目。

## 設計原則

* 使用 `--yc-*` 作為 booking 網站的 source of truth。
* 不再輸出 `--bk-*` 或 `--yui-*` 相容 alias；主站與 booking 新增或重構 SCSS 都必須直接使用 `--yc-*`。
...
`請讀取 booking/booking-style-tokens.md，並以其中的 --yc-* token 作為 booking 網站 source of truth。不要新增與 --yc-* 衝突的色彩系統；不要新增 --bk-* 或 --yui-* alias。請檢查互動元件是否有 44px touch target、可見 focus-visible、150ms 到 300ms motion、prefers-reduced-motion、WCAG AA 對比，以及 hover 不造成 layout shift。不要修改 HTML class 或 JavaScript selector，除非需求明確要求。`
