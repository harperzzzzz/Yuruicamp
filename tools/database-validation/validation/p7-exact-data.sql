...
-- Used only for backup/restore equality. Unlike the A/B path comparison this
-- includes operational timestamps, Flyway history and current sequence state.
CREATE TEMP TABLE p7_exact_fingerprints (
...
BEGIN
  ...
  LOOP
    EXECUTE format(
      'SELECT count(*), md5(coalesce(string_agg(to_jsonb(source_row)::text, E''\\n'' ORDER BY to_jsonb(source_row)::text), '''')) FROM %I.%I source_row',
      relation.schema_name, relation.relation_name
    ) INTO count_value, hash_value;
    INSERT INTO p7_exact_fingerprints VALUES (
      relation.schema_name || '.' || relation.relation_name,
      count_value,
      hash_value
    );
...
INSERT INTO p7_exact_fingerprints
SELECT schemaname || '.' || sequencename,
       coalesce(last_value, 0),
       md5(concat_ws('|', start_value, min_value, max_value, increment_by, cycle, cache_size, last_value))
FROM pg_sequences
