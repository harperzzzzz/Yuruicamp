...
$out.Add('')
$out.Add('> 來源：`docs/schema_copy.sql`（V714 驗證後快照）。本文件由 `tools/database-validation/generate-schema-guide.ps1` 產生；欄位描述以名稱與 DDL 約束解讀，業務規則以 SQL 為最終準則。')
$out.Add('')
$out.Add('## 先說結論')
$out.Add('')
$out.Add('- **資料庫**：PostgreSQL。證據包含 `pg_catalog`、`plpgsql`、`jsonb`、`timestamp with time zone`、`generate_series` 與 PostgreSQL dump 格式。')
$out.Add('- **Schema 定義方式**：資料庫優先（database-first）。`docs/schema_copy.sql` 是可重建新資料庫的最終快照；既有資料庫只能依 `backend/src/main/resources/db/migration/V001…V714` 的 Flyway migration 升級。')
