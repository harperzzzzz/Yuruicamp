...
> 狀態：**COMPLETE（2026-07-14）**；`P0` 至 `P7` 全部完成。P7 的正式環境觀察與組織簽核由專案負責人明確豁免，未偽造為已觀察證據
> 目標資料庫：`PostgreSQL`  
> 基準日期：`2026-07-13`  
...
7. `P0` 已建立與凍結基線相同的 `schema_copy.sql` 佔位檔；它不是最終快照，只有 P7 contract 全部通過後才能由最終結構重新產生。
...
| ------- | --------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | ---------- | ---------------- | -------- |
| D-001   | 會員等級代碼與門檻                      | 保留 `explorer`、`guide`、`master`；以完成且未退款訂單總額衍生：`< 12000`、`12000-27999`、`>= 28000`，不存入 `customers`                                                           | 已定案     | P1、P4、P7 | 無               | APPROVED |
| D-002   | 是否只允許已購買會員評論                | 正式評論只允許已購買會員建立，`order_item_id` 是唯一權威關聯；`REV031` 進正式表，其餘 `37` 筆進唯讀 `legacy_reviews`                                                              | 已定案     | P4、P6、P7 | 無               | APPROVED |
| D-003   | 優惠券使用量認列時間                    | `payment_status = paid` 且訂單為 `unshipped`、`shipped`、`completed` 時認列；全額退款或核准退貨以冪等調整 ledger 恢復一次額度                                                      | 已定案     | P4、P7     | 無               | APPROVED |
| D-004   | 平日與假日的權威來源                    | 版本化台灣日曆／假日設定表為權威；前端只顯示，後端建立交易時重新計算                                                                                                               | 已定案     | P4、P5、P7 | 無               | APPROVED |
...
| D-007   | 商品與租借是否共用分類                  | 共用 `product_categories`，由 `equipment_items.category_id` 引用                                                                                                                  | 已定案     | P2、P3     | 無               | APPROVED |
| D-008   | 主檔是否允許物理刪除                    | 商品、裝備、租借、會員、營區、品牌只允許停用；有交易引用時禁止物理刪除                                                                                                            | 已定案     | P1 至 P7   | 無               | APPROVED |
...
| D-011   | 販售保留生命週期、逾期與出貨庫位        | 線上未付款訂單 `30` 分鐘逾期且每分鐘掃描；COD 保留到出貨／取消；主倉後按分店代碼分配、允許拆庫位；退貨先進待檢庫位再由異動轉可售／維修／損壞                                      | 已定案     | P4、P5、P7 | 無               | APPROVED |
...
| `product_categories`                                       | `[設定]` | 分類身份、名稱、排序                 | `name UNIQUE`；被引用時 `RESTRICT`                                                                     | 商品／租借分類文字                                |
...
| `product_stock_summary`                                    | `[衍生] VIEW` | 商品實體總量、active 保留量與可售量 | 真相為 `inventory_stocks` 與 `product_stock_reservations`；禁止直接寫入                               | `products.totalStock` 相容輸出                    |
...
- D-011 已核准：線上付款保留期限固定為 `30` 分鐘並由每分鐘排程處理；COD 保留至出貨或取消；依主倉、分店代碼排序分配且允許拆分多庫位；退貨一律先進 `inspection` 庫位，再以異動單轉入可售、維修或損壞庫位。
...
| `coupon_usage_adjustments` | `[交易]`       | 用券紀錄、恢復／重新認列類型、數量差、原因、冪等鍵                                 | 用券紀錄 `RESTRICT`；`idempotency_key UNIQUE`；數量只允許 `-1` 或 `1`                           | 退款／核准退貨事件                                  |
| `coupon_usage_stats`       | `[衍生] VIEW`  | 已使用與剩餘數量                                                                   | 真相為有效訂單的 `order_coupons`                                                               | `coupons.used` 相容輸出                             |
| `calendar_dates`           | `[設定]`       | 台灣日曆日期、假日旗標、名稱、來源版本、生效與更新時間                             | 日期 PK；版本與來源不可空；交易服務只讀，日曆管理流程版本化更新                                 | 平假日價格計算                                      |
...
| `booking_policy_availability_statuses` | `[關聯][設定]` | 政策與可用性狀態                                                    | 複合 PK；政策刪除 `CASCADE`                                                                                      | `availabilityStatuses[]`       |
...
本節與 `docs/database/p0-target-dictionary.json` 共同構成產生後續 migration 與最終 `docs/schema_copy.sql` 的唯一結構來源。JSON 字典由 `admin/scripts/generate-p0-target-dictionary.cjs` 產生並由 P0 稽核逐欄驗證；本節的 `products` 是呈現格式範例，不表示其他物件可以省略欄位定義。

完整字典涵蓋本節 5.1 至 5.5 的全部目標 table、view 與 query；任何目標物件若未出現在 JSON、欄位缺少型別／NULL／DEFAULT／用途／來源、約束或索引沒有固定名稱，P0 稽核即失敗。
...
├─ V100__p1_expand_master_data.sql
├─ V110__p1_backfill_master_data.sql
├─ V120__p1_constraints_master_data.sql
...
> 狀態：**COMPLETE（2026-07-14）**。`V100`、`V110`、`V120` 已在隔離 PostgreSQL 18.4 由 Flyway 依序套用並通過重啟驗證；結構、資料、相容 DTO 與業務規則驗證皆為 `0` issue，P2 物件為 `0`。完整證據見 `docs/database/p1-execution-report.md`。
...
> 狀態：**COMPLETE（2026-07-14）**。`V200`、`V210`、`V220` 已在隔離 PostgreSQL 18.4 由 Flyway 從 `V001` 全量套用並通過第二次啟動；商品、裝備、變體、標籤、規格與販售庫存的結構、資料、相容 DTO 及業務規則驗證皆為 `0` issue，P3 物件為 `0`。映射見 `docs/database/p2-mapping-report.md`，完整證據見 `docs/database/p2-execution-report.md`。
...
> 狀態：**COMPLETE（2026-07-14）**。`V300`、`V310`、`V320` 已在隔離 PostgreSQL 18.4 由 Flyway 從 `V001` 全量套用並通過第二次啟動；租借 SKU、獨立租借變體、`C002-C009` 營區庫位映射、`C001-C009` 實體庫存、listing 與相容 view 的結構、資料、DTO 及業務規則驗證皆為 `0` issue，P4 migration 為 `0`。正式 reservation ledger 依核准 allocation 留在 P4，正式租借最低庫存表留在 P5；P3 已保留可唯一解析的來源證據。映射見 `docs/database/p3-mapping-report.md`，完整證據見 `docs/database/p3-execution-report.md`。
...
> 狀態：**COMPLETE（2026-07-14）**。`V400`、`V410`、`V420` 已在隔離 PostgreSQL 18.4 由 Flyway 從 `V001` 全量套用並通過第二次啟動；訂單、優惠券、預約、日曆、狀態歷程、兩張 reservation ledger 與衍生 view 的結構、資料、財務、DTO 相容及業務規則驗證皆為 `0` issue。P4-D01 至 P4-D04 已全部核准；40 筆租借來源明細已對帳為 21 筆正式 reservation 與 19 筆核准 quarantine，既有販售訂單 reservation 為 0。完整證據見 `docs/database/p4-execution-report.md`。
...
> 狀態：**COMPLETE（2026-07-14）**。`V500`、`V510`、`V520` 已由 Flyway 從 `V001` 全量套用至 `v520` 並通過第二次啟動；政策、block／closure、兩類最低庫存、單一領域異動、conversion、reservation lifecycle、DTO 與可用性查詢驗證皆為 `0` issue。100 張／141 筆舊異動已解析為 171 張單一領域表頭、65 筆販售明細、44 筆租借明細、31 筆 conversion 與 1 筆 `NO_RENTAL_VARIANT` 具原因隔離；32 筆商店到營區項目全部完成分類，未分類與猜測變體皆為 `0`。SQL 與既有 JavaScript 可用性 4,745 列差異為 `0`。完整證據見 `docs/database/p5-execution-report.md`。P6 尚未開始。
...
> 狀態：**COMPLETE（2026-07-14）**。`V600`、`V610`、`V620` 已由 Flyway 從 `V001` 全量套用至 `v620` 並通過第二次啟動；`REV031` 唯一遷移至正式評論的 `order_item_id = 418`，其餘 `37` 筆以 `NO_ORDER_ID` 原因進唯讀 `legacy_reviews`，模糊、未分類與未遷移數量皆為 `0`。6 篇文章已正規化為 18 個標籤、41 個有序內容區塊（18 text／12 heading／11 product）與 16 個推薦商品；官方回覆 relation／欄位／DTO 數量為 `0`。後台異動 DTO 只讀 P5 `inventory_movement_items_view`。完整證據見 `docs/database/p6-execution-report.md`；後續 P7 亦已完成。
...
> 狀態：**COMPLETE WITH OWNER WAIVER（2026-07-14）**。專案負責人明確豁免 7 日／100 筆正式環境觀察、production telemetry／效能、部署後證據與組織簽核；這些欄位維持 `WAIVED`，未偽造成通過。`V700` 已執行，`movement_migration_map` 的 141 筆資料已封存至唯讀 `migration` schema，暫時相容 view 已移除。A（V001-V700）、B（P0 baseline 後 V100-V700）與 C（最終快照）canonical catalog 完全一致；全域驗證、Java 25 Spring Boot、備份還原、效能與 68 項 browser smoke 全數通過。最終 `docs/schema_copy.sql` 已產生，`docs/schema.sql` 仍保留為凍結基線。詳見 `docs/database/p7-execution-report.md`、`docs/database/p7-waiver-decision.md` 與 `docs/database/p7-observation-evidence.json`。
...
8. `schema_copy.sql` 已在 P7 完成後由 V700 資料庫產生，並通過 A／B／C canonical catalog 等價驗證；它是空庫快照，不是既有資料庫的升級方式。
...
- 最終決策：保留目前實際資料與前端使用的代碼；`explorer < 12000`、`guide = 12000-27999`、`master >= 28000`。
- 認列規則：只加總 `status = completed` 且 `payment_status = paid`、未退款的訂單 `total`；不將快照 `customers.total_spent` 當真相。
- 狀態：APPROVED。
- 影響：P1、P4、P7。
...
- 最終流程：購物車不保留；訂單正式送出建立 active reservation；出貨時扣除實體庫存並轉 fulfilled；取消或逾期轉 released／expired；退貨以庫存異動決定實際入庫位置。
- 狀態對應：`unshipped` 佔用、`shipped` 與 `completed` 已履行、`cancelled` 釋放、`returned` 不自動增加可售量。
- 逾期：線上未付款訂單建立後 `30` 分鐘到期，由後端每分鐘的排程以冪等方式轉 `expired`；COD 不套用付款逾期，保留至出貨或取消。
- 庫位：先分配主倉，再按分店代碼排序；允許同一訂單品項拆成多筆不同庫位 reservation，總和必須等於訂單品項數量。
- 退貨：一律先進 `inspection` 待檢庫位；營運人員檢查後以正式庫存異動轉入可售、維修或損壞庫位，不由 `returned` 狀態直接回補。
- 必要 schema：`order_status` 新增 `cancelled`；線上付款 reservation 的 `expires_at` 必填，COD 可空；每筆 reservation 保存分配庫位及冪等鍵。
- 狀態：APPROVED。
- 影響：P4、P5、P7。
...
`P0` 至 `P7` 已完成。正式環境觀察與組織簽核依專案負責人指示明確豁免；若日後要部署至正式環境，仍建議重新執行 `p7-cutover-runbook.md` 的觀察、telemetry、正式規模效能與簽核步驟。既有資料庫只能透過 Flyway migration 升級；`docs/schema_copy.sql` 僅供建立空庫及結構比對。`docs/schema.sql` 維持凍結基線，未被自動取代。
