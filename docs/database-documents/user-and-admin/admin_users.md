...
## 關聯與資料流
admin_users
├─ 1:N admin_user_permissions  （個別權限覆寫，刪除帳號時 CASCADE）
├─ N:M admin_permissions       （透過 admin_user_permissions）
...
* admin_user_permissions：保存個別管理員相對於角色預設權限的允許或禁止設定。
* admin_permissions：保存 10 個後台功能的 view／edit 權限代碼；角色預設權限另由 admin_role_permissions 關聯。
...
admin_user_permissions 對 admin_users 採 `ON UPDATE CASCADE ON DELETE CASCADE`，刪除帳號時會移除個別權限覆寫。業務歷程、庫存異動、停售與公休外鍵則採 `ON UPDATE CASCADE ON DELETE RESTRICT`；只要仍有歷史紀錄，就不能刪除該管理員。

### 資料流程
* 後台人員建立後：
- 在 admin_users 建立人員基本資料與角色。
- 依 role 從 admin_role_permissions 取得預設權限，再套用 admin_user_permissions 的個別覆寫。
...
* name                管理員姓名
* email               登入或聯絡信箱。
                      唯一索引 `uq_admin_users_email_lower` 使用 `lower(email)`，因此不允許只差大小寫的重複信箱。
...
*uq_admin_users_email_lower ON (lower(email))*：
以不區分大小寫的方式保證管理員信箱唯一。



## 運作模式
* admin_users 是三種粗粒度角色的帳號主檔；權限明細由 admin_permissions、admin_role_permissions、admin_user_permissions 保存。
admin：預期為完整後台管理權限。
...
admin_users.role
        ↓
admin_role_permissions 角色預設權限
        ↓
admin_user_permissions.allowed 個別覆寫
        ↓
組合為前端 10 個功能頁面的 view／edit 權限物件
...
* 後台登入
    ...
    * 目前實際執行時：
        ...
        - `latest_schema.sql` 已有帳號與權限表結構，但目前沒有對應的登入 Controller／Service／Repository。
...
* 高風險｜資料庫已建立角色預設權限與個別權限覆寫模型，但前端仍只使用 localStorage.adminEmployees；正式登入與權限 API 完成前，兩端權限不會同步。
