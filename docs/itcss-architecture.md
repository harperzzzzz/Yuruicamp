...
Booking 子系統已改用 SCSS `@use` 與主站 ITCSS 對齊，公開頁入口為 `booking/css/booking-main.scss`，會員中心入口為 `booking/css/member-center-main.scss`：
...
Booking runtime 頁面仍載入編譯輸出 `booking/css/booking-main.css` 與 `booking/css/member-center-main.css`，以保留現有 HTML 路徑相容。`booking/css/booking.css` 已收斂為 legacy bridge，不再由 runtime 頁面載入。
