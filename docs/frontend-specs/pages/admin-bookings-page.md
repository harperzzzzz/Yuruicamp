# AdminBookingsPage Spec

**Status:** Draft
**Category:** Page
**Design Ref:** N/A - derived from existing source file `admin/partials/bookings.html`

---

## Overview

Admin booking-management partial with date filters, booking table, mobile cards, and status operations. Use for campsite and rental reservation operations. Keep confirmation and cancellation actions distinct.
...
export interface AdminBookingsPageData {
  title: string;
  sourcePath: 'admin/partials/bookings.html';
  keyAreas: string[];
  blocks?: ContentBlock[];
}
...
export interface AdminBookingsPageProps {
  ...
  data: AdminBookingsPageData; // Initial page content, records, or mounted section metadata.
  ...
  onRefresh?: (sourcePath: 'admin/partials/bookings.html') => void;
...
|---------|-------|-------------|
| Default | `shell="admin"` | Matches the current `admin/partials/bookings.html` layout and shared CSS. |
...
<AdminBookingsPage
  shell="admin"
  data={{
    title: 'AdminBookingsPage',
    sourcePath: 'admin/partials/bookings.html',
    keyAreas: 'bookingPeriodBtns, bookingsTable, bookingsTableBody'.split(', '),
  }}
/>
```
