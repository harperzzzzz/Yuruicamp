...
**Status:** Draft
**Category:** Page
**Design Ref:** N/A - derived from existing source file `booking/pages/camp-detail.html`

---

## Overview

Campground detail page with gallery, description, date summary, guest count, zone list, and reservation CTA. Use for campsite inspection and stay configuration. Keep pricing, holiday weekday labels, and zone availability visible.
...
export interface CampDetailPageData {
  title: string;
  sourcePath: 'booking/pages/camp-detail.html';
  keyAreas: string[];
  blocks?: ContentBlock[];
}
...
export interface CampDetailPageProps {
  ...
  data: CampDetailPageData; // Initial page content, records, or mounted section metadata.
  ...
  onRefresh?: (sourcePath: 'booking/pages/camp-detail.html') => void;
...
|---------|-------|-------------|
| Default | `shell="booking"` | Matches the current `booking/pages/camp-detail.html` layout and shared CSS. |
...
<CampDetailPage
  shell="booking"
  data={{
    title: 'CampDetailPage',
    sourcePath: 'booking/pages/camp-detail.html',
    keyAreas: 'campHeader, campGallery, dateSummary, zoneList'.split(', '),
  }}
/>
```
...
- Source file: `booking/pages/camp-detail.html`.
...
- Key UI areas: campHeader, campGallery, dateSummary, zoneList.
