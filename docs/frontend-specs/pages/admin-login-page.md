...
Seller admin login page with employee ID, password field, visibility toggle, and error display. Use for admin authentication. Keep errors explicit and avoid exposing credential details.
...
export interface AdminLoginPageProps {
  ...
  data: AdminLoginPageData; // Initial page content, records, or mounted section metadata.
  ...
  onRefresh?: (sourcePath: 'admin/login.html') => void;
...
|---------|-------|-------------|
| Default | `shell="admin"` | Matches the current `admin/login.html` layout and shared CSS. |
...
<AdminLoginPage
  ...
  data={{
    ...
    sourcePath: 'admin/login.html',
    keyAreas: 'loginForm, togglePassword, loginError'.split(', '),
  }}
/>
```
