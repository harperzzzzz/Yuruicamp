...
**狀態：** 草稿
**類別：** 頁面
**設計參考：** 無－依據既有原始檔 `pages/faq.html` 整理

---

## 概覽

常見問題頁面，包含搜尋、分類導覽、手風琴式答案與回饋表單。

此頁用於支援使用者自助查詢。必須讓搜尋、分類錨點與回饋驗證功能維持良好的無障礙性。
...
export interface FaqPageProps {
  ...
  data: FaqPageData; // 初始頁面內容、資料紀錄或掛載區塊的中繼資料。
  ...
  onRefresh?: (sourcePath: 'pages/faq.html') => void;
...
| 版本  | Props                | 說明                                   |
| --- | -------------------- | ------------------------------------ |
| 預設  | `shell="main"`       | 符合目前 `pages/faq.html` 的版面與共用 CSS。    |
...
<FaqPage
  ...
  data={{
    title: 'FaqPage',
    sourcePath: 'pages/faq.html',
    keyAreas: 'faqSearchInput, categoryNav, faqAccordion, feedbackForm'.split(', '),
  }}
/>
```
...
* 關鍵 UI 區域：`faqSearchInput`、`categoryNav`、`faqAccordion`、`feedbackForm`。
