...
**Status:** Draft
**Category:** Page
**Design Ref:** N/A - derived from existing source file `booking/pages/camp-rental.html`

---

## Overview

Rental add-on page with booking summary, recommended equipment, rental grid, rental cart, subtotal, and continue action. Use after campsite selection to attach rental equipment. Preserve skip and continue paths.
...
export interface CampRentalPageData {
  title: string;
  sourcePath: 'booking/pages/camp-rental.html';
  keyAreas: string[];
  blocks?: ContentBlock[];
}

export interface CampRentalPageProps {
  // Required props
  shell: 'booking'; // Page shell variant used by this source page.
  data: CampRentalPageData; // Initial page content, records, or mounted section metadata.
  ...
  onRefresh?: (sourcePath: 'booking/pages/camp-rental.html') => void;
...
|---------|-------|-------------|
| Default | `shell="booking"` | Matches the current `booking/pages/camp-rental.html` layout and shared CSS. |
...
<CampRentalPage
  shell="booking"
  data={{
    title: 'CampRentalPage',
    sourcePath: 'booking/pages/camp-rental.html',
    keyAreas: 'bookingSummaryBar, recommendationBanner, rentalGrid, rentalCartList'.split(', '),
  }}
/>
```
