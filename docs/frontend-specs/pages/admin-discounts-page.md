# AdminDiscountsPage Spec

**Status:** Draft
**Category:** Page
**Design Ref:** N/A - derived from existing source file `admin/partials/discounts.html`

---

## Overview

Admin coupon partial with add coupon form, generated code, discount type switch, quantity/date fields, and coupon table. Use for coupon creation and management. Keep date and discount validation visible.
...
export interface AdminDiscountsPageData {
  title: string;
  sourcePath: 'admin/partials/discounts.html';
  keyAreas: string[];
  blocks?: ContentBlock[];
}
...
export interface AdminDiscountsPageProps {
  ...
  data: AdminDiscountsPageData; // Initial page content, records, or mounted section metadata.
  ...
  onRefresh?: (sourcePath: 'admin/partials/discounts.html') => void;
...
|---------|-------|-------------|
| Default | `shell="admin"` | Matches the current `admin/partials/discounts.html` layout and shared CSS. |
...
<AdminDiscountsPage
  shell="admin"
  data={{
    title: 'AdminDiscountsPage',
    sourcePath: 'admin/partials/discounts.html',
    keyAreas: 'addCouponForm, discountTypeSwitch, couponsTableBody'.split(', '),
  }}
/>
```
...
- Source file: `admin/partials/discounts.html`.
