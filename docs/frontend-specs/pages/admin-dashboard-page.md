# AdminDashboardPage Spec

**Status:** Draft
**Category:** Page
**Design Ref:** N/A - derived from existing source file `admin/dashboard.html`

---

## Overview

Admin dashboard shell with sidebar, topbar, content loader, shared modals, and toast region. Use as the host for all admin partial pages. Preserve partial loader and section permission gates.
...
export interface AdminDashboardPageData {
  title: string;
  sourcePath: 'admin/dashboard.html';
  keyAreas: string[];
  blocks?: ContentBlock[];
}
...
export interface AdminDashboardPageProps {
  ...
  data: AdminDashboardPageData; // Initial page content, records, or mounted section metadata.
  ...
  onRefresh?: (sourcePath: 'admin/dashboard.html') => void;
...
|---------|-------|-------------|
| Default | `shell="admin"` | Matches the current `admin/dashboard.html` layout and shared CSS. |
...
<AdminDashboardPage
  ...
  data={{
    ...
    sourcePath: 'admin/dashboard.html',
    keyAreas: 'adminSidebar, mobileSidebar, contentArea, adminModals'.split(', '),
  }}
/>
```
...
- Key UI areas: adminSidebar, mobileSidebar, contentArea, adminModals.
