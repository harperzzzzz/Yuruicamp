# IndexRedirectPage Spec

**Status:** Draft
**Category:** Page
**Design Ref:** N/A - derived from existing source file `index.html`

---

## Overview

Entry page that mirrors the buyer home shell and loads shared main-site components. Use when the app needs a root URL entry that can still hydrate shared login, cart, blog, branch, and member-center scripts.
...
export interface IndexRedirectPageData {
  title: string;
...
export interface IndexRedirectPageProps {
  // Required props
  shell: 'main'; // Page shell variant used by this source page.
  data: IndexRedirectPageData; // Initial page content, records, or mounted section metadata.
  ...
  onRefresh?: (sourcePath: 'index.html') => void;
...
|---------|-------|-------------|
| Default | `shell="main"` | Matches the current `index.html` layout and shared CSS. |
...
<IndexRedirectPage
  shell="main"
  data={{
    title: 'IndexRedirectPage',
    sourcePath: 'index.html',
    keyAreas: 'hero, modal, cart, shared header'.split(', '),
  }}
/>
```
...
- Shared CSS source: `css/main.css`.
- Shared components: components/header.partial, components/footer.partial.
- Key UI areas: hero, modal, cart, shared header.
