# ProductDetailPage 商品詳細頁規格

**狀態：** 草稿
**類別：** 頁面
**設計參考：** 無－依據既有原始檔 `pages/product-detail.html` 整理

---

## 概覽

商品詳細頁，包含商品圖片集、購買操作、規格、評價與推薦商品。

此頁用於讓使用者檢視商品並進行購買決策。必須清楚保留圖片集、數量選擇、加入購物車與直接結帳的流程。
...
export interface ProductDetailPageData {
  title: string;
  sourcePath: 'pages/product-detail.html';
  keyAreas: string[];
  blocks?: ContentBlock[];
}
...
export interface ProductDetailPageProps {
  ...
  data: ProductDetailPageData; // 初始頁面內容、資料紀錄或掛載區塊的中繼資料。
  ...
  onRefresh?: (sourcePath: 'pages/product-detail.html') => void;
...
| 版本  | Props                | 說明                                           |
| --- | -------------------- | -------------------------------------------- |
| 預設  | `shell="main"`       | 符合目前 `pages/product-detail.html` 的版面與共用 CSS。 |
...
<ProductDetailPage
  ...
  data={{
    ...
    sourcePath: 'pages/product-detail.html',
    keyAreas: 'gallery, productInfo, purchasePanel, reviews'.split(', '),
  }}
/>
```
...
* 原始檔：`pages/product-detail.html`。
* 共用 CSS 來源：`css/main.css`。
* 共用元件：`components/header.partial`、`components/footer.partial`。
* 關鍵 UI 區域：`gallery`、`productInfo`、`purchasePanel`、`reviews`。
* 「立即購買」以商品 ID 與 variant ID 檢查購物車；相同品項已存在時沿用原數量並直接前往確認背包，不再次累加。
* 「加入購物車」維持既有行為，相同商品規格會依使用者輸入數量累加。
