# BranchesPage 分店與合作夥伴頁規格

**狀態：** 草稿
**類別：** 頁面
**設計參考：** 無－依據既有原始檔 `pages/branches.html` 整理

---

## 概覽

實體門市與合作夥伴頁面，包含分店清單、地圖、路線規劃、合作夥伴卡片與詳細資訊 Modal。

此頁用於協助使用者尋找實體門市，並查看合作露營地的相關資訊。必須保留地圖載入失敗時的替代處理，以及 Modal 的各種狀態。
...
export interface BranchesPageData {
  title: string;
  sourcePath: 'pages/branches.html';
  keyAreas: string[];
  blocks?: ContentBlock[];
}
...
export interface BranchesPageProps {
  ...
  data: BranchesPageData; // 初始頁面內容、資料紀錄或掛載區塊的中繼資料。
  ...
  onRefresh?: (sourcePath: 'pages/branches.html') => void;
...
| 版本  | Props                | 說明                                     |
| --- | -------------------- | -------------------------------------- |
| 預設  | `shell="main"`       | 符合目前 `pages/branches.html` 的版面與共用 CSS。 |
...
<BranchesPage
  ...
  data={{
    title: 'BranchesPage',
    sourcePath: 'pages/branches.html',
    keyAreas: 'branchList, mapWrap, partnersGrid, partnerModal'.split(', '),
  }}
/>
```
