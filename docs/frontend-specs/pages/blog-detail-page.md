# BlogDetailPage 部落格文章詳細頁規格

**狀態：** 草稿
**類別：** 頁面
**設計參考：** 無－依據既有原始檔 `pages/blog-detail.html` 整理

---

## 概覽

文章閱讀頁，包含麵包屑、主視覺圖片、作者資訊、文章正文、內嵌商品卡片與相關文章。

此頁用於呈現編輯內容與商品教育資訊。必須維持適合閱讀的文字行寬，並讓商品卡片具備清楚的互動提示。
...
export interface BlogDetailPageData {
  title: string;
  sourcePath: 'pages/blog-detail.html';
  keyAreas: string[];
  blocks?: ContentBlock[];
}
...
export interface BlogDetailPageProps {
  // 必填 Props
  shell: 'main'; // 此原始頁面使用的頁面外殼類型。
  data: BlogDetailPageData; // 初始頁面內容、資料紀錄或掛載區塊的中繼資料。
  ...
  onRefresh?: (sourcePath: 'pages/blog-detail.html') => void;
...
| 版本  | Props                | 說明                                        |
| --- | -------------------- | ----------------------------------------- |
| 預設  | `shell="main"`       | 符合目前 `pages/blog-detail.html` 的版面與共用 CSS。 |
...
<BlogDetailPage
  ...
  data={{
    ...
    sourcePath: 'pages/blog-detail.html',
    keyAreas: 'breadcrumb, articleHero, articleMeta, articleContent, relatedArticles'.split(', '),
  }}
/>
```
...
* 原始檔：`pages/blog-detail.html`。
* 共用 CSS 來源：`css/main.css`。
* 共用元件：`components/header.partial`、`components/footer.partial`。
* 關鍵 UI 區域：`breadcrumb`、`articleHero`、`articleMeta`、`articleContent`、`relatedArticles`。
