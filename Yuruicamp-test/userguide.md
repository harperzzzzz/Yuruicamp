...
│   ├── js/                        ← 後台 JS（core + 6 個功能模組）
│   ├── partials/                  ← 六個功能模組 HTML 片段
│   └── data/                      ← 後台專用 Mock JSON（與買家 data/ 分離）
│
├── pages/                         ← 所有功能 HTML 頁面（共 12 個，買家前台）
...
| `admin/data/products.json` | **後台商品**（10 筆），含庫存數量、上架狀態 | `[{ id, name, price, category, stock, status, image }]` |
...
> - 後台 **低庫存警告** 閾值：`stock < 5`（在 `analytics.js` 中定義）
...
| 改後台品牌 Accent 色 | `admin/css/admin.css` → `--admin-accent`（預設 `#244d4d`）|
| 改後台 Toast 行為 | `admin/js/core.js` → `showAdminToast()` |
| 改登入驗證邏輯 | `admin/login.html` 底部 script（目前為任意非空帳密均可登入）|
| 改後台登出行為 | `admin/js/core.js` → logout 按鈕事件監聽 |
| 修改訂單資料 | `admin/data/orders.json` |
| 修改商品庫存 / 上架狀態 | `admin/data/products.json` → `stock` / `status` 欄位 |
...
| 改低庫存警告閾值 | `admin/js/analytics.js` → `stock < 5`（改成需要的數字）|
| 改圖表樣式 / 資料 | `admin/js/analytics.js` → Chart.js 設定物件 |
| 新增後台 Sidebar 功能模組 | `admin/dashboard.html`（加 Sidebar 連結）+ 新增 `admin/partials/xxx.html` + `admin/js/xxx.js` |
...
2. 瀏覽器是否開了無痕模式（某些無痕設定會阻擋 `sessionStorage`）？→ 改用一般模式
3. 打開 DevTools → Application → Session Storage，確認有 `adminLoggedIn: "true"` 這個鍵
...
使用者開啟 admin/login.html
  ↓ 輸入帳號 + 密碼（任意非空字串均可，Mock 驗證）
  ↓ 寫入 sessionStorage：
      adminLoggedIn = "true"
      adminName = "輸入的帳號"
  ↓ 0.8 秒後自動跳轉到 admin/dashboard.html
  
admin/dashboard.html 載入時
  ↓ core.js 執行 Auth 守衛
      如果 sessionStorage.adminLoggedIn !== "true"
      → 立即跳回 admin/login.html（防止未授權直接開啟 dashboard）
  ↓ 預設載入「數據總覽」模組（analytics）
  ↓ 使用者點 Sidebar 各項目 → 動態載入對應 partial
  
登出
  ↓ 點右上角頭像 → 「登出」選項
  ↓ 清除 sessionStorage → 跳回 admin/login.html
```

> 💡 **為什麼用 sessionStorage 而不是 localStorage？**  
> 因為後台通常需要「關閉分頁就登出」的行為，`sessionStorage` 在分頁關閉後自動清除，而 `localStorage` 會持久保留。
...
- 注入完成後，呼叫對應模組的初始化函數（如 `window.initAnalytics()`）

```javascript
// core.js 的 loadSection() 運作方式（簡化版）
function loadSection(name) {
  $('#contentArea').load(`partials/${name}.html`, function() {
    // 載入完成後，呼叫對應模組的初始化
    if (name === 'analytics') window.initAnalytics();
    if (name === 'orders')    window.initOrders();
    // ... 以此類推
  });
}
```
...
|---------|-------------|---------|---------|---------|
| 數據總覽 | `partials/analytics.html` | `js/analytics.js` | `orders.json` + `products.json` | 今日營業額 KPI、待出貨筆數、低庫存預警、折線圖、甜甜圈圖 |
| 訂單管理 | `partials/orders.html` | `js/orders.js` | `orders.json` | 訂單表格、狀態篩選、標記出貨、訂單詳情 Modal |
| 商品管理 | `partials/products.html` | `js/products.js` | `products.json` | 商品表格、庫存行內編輯（點擊直接改數字）、上架/下架切換、新增商品 Modal |
| 會員管理 | `partials/customers.html` | `js/customers.js` | `customers.json` | Accordion 折疊清單、會員等級 / 點數 / 優惠券行內編輯 |
| 折扣管理 | `partials/discounts.html` | `js/discounts.js` | `coupons.json` | 優惠券列表、新增 / 啟用 / 停用優惠券、一鍵產生隨機碼 |
| 評論管理 | `partials/reviews.html` | `js/reviews.js` | `reviews.json` | 評論卡片、依星等篩選、填寫並送出回覆 |

---

### 後台 CSS 變數

後台有自己的一套設計 Token，定義在 `admin/css/admin.css` 的 `:root` 區塊：

```css
:root {
  /* Sidebar 配色 */
  --admin-sidebar-bg:    #1e2329;  /* 深炭黑 Sidebar 背景 */
  --admin-sidebar-text:  #adb5bd;  /* Sidebar 文字（淺灰）*/
  --admin-sidebar-hover: #2c3440;  /* Sidebar 選項 hover 色 */
  --admin-sidebar-active:#244d4d;  /* Sidebar 選中項目（品牌深青綠）*/

  /* 品牌 Accent 色 */
  --admin-accent:        #244d4d;  /* 按鈕、連結、強調色 */
  --admin-accent-hover:  #316868;  /* Accent hover 狀態 */

  /* 內容區 */
  --admin-content-bg:    #f0f2f5;  /* 右側內容區淺灰底 */
}
```

> 💡 若要更換後台主題色，只需修改這幾個 CSS 變數的色碼，整個後台的配色會統一改變。

---

### 後台 JS 共用函數

後台有自己的工具函數，都定義在 `admin/js/core.js`，只在後台使用（不影響買家前台）：

```javascript
// 顯示後台 Toast 提示（右上角彈出）
showAdminToast(message, type)
// type: 'success' | 'danger' | 'warning' | 'info'
// 範例：showAdminToast('訂單已標記出貨', 'success')

// 載入功能模組（注入 partial + 初始化）
loadSection(name)
// name: 'analytics' | 'orders' | 'products' | 'customers' | 'discounts' | 'reviews'
// 由 Sidebar 點擊事件自動呼叫，通常不需要手動呼叫
```
...
```
1. 在 admin/partials/ 新增 xxx.html（只寫內容區的 HTML，不需要 <html> 外殼）

2. 在 admin/js/ 新增 xxx.js，包含：
   window.initXxx = function() {
     // 讀取 JSON、渲染 HTML、綁定事件
   }

3. 在 admin/dashboard.html：
   a. Sidebar 加一個 <a> 選項，data-section="xxx"
   b. 底部 <script> 引入新的 xxx.js

4. 在 admin/js/core.js 的 loadSection() 函數裡，
   加入 if (name === 'xxx') window.initXxx();

5. （可選）在 admin/data/ 新增 xxx.json，存放模組的 Mock 資料
```
