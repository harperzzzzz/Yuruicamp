...
async function fetchBranches() {
  ...
  const path =
    (window.MockDataPaths && window.MockDataPaths.branches) ||
...
async function fetchCampgrounds() {
  ...
  const path =
    (window.MockDataPaths && window.MockDataPaths.campgrounds) ||
...
function getPartnerBookingHref(campgroundId) {
  // 從 storefront/pages 回到 frontend 根再進 booking / Two levels up from storefront/pages
